...
$X_1\sim Poi(\lambda_1), X_2\sim Poi(\lambda_2)$. $X_1\perp X_2$, $Y=X_1 + X_2$
...
Notation $X,Y|\{ Z = k \}\stackrel{iid}{\sim}$ means that given $Z=k$, $X$ and $Y$ are _conditionally independent_, and they follow certain distribution.
